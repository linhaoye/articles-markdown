...
static int
vcasecmp(const struct vec *v1, const struct vec *v2)
{
	register const char	*s1, *s2, *e;
	int			equal = 0;

	if (v1->len == v2->len) {
		for (s1 = v1->ptr, s2 = v2->ptr, e = s1 + v1->len;
		    s1 < e; s1++, s2++)
...
static const char *
vchr(const struct vec *v, char ch)
{
 ...
 */
static int
htparse(const char *s, int len, struct hti *info, hcb_t cb, void *data)
{
...
accesslog(const struct conn *c, int status)
{
	...
	if ((fp = fopen(logfile, "a"))) {
		...
		(void) fprintf(fp, "%.15s - %s [%s] \"%.*s %.*s %.*s\" %d\n",
		    ...
		    c->user[0] == '\0' ? "-" : c->user,
		    date,
		    c->hti.method.len, (char *) c->hti.method.ptr,
		    c->hti.url.len, (char *) c->hti.url.ptr,
		    c->hti.proto.len, (char *) c->hti.proto.ptr, status);
		(void) fclose(fp);
	}
}
 ...
 */
static DWORD WINAPI
stdoutput(struct threadparam *tp)
{
	int			n, k, sent, stop = 0;
	char			buf[8192];
 ...
 */
static DWORD WINAPI
stdinput(struct threadparam *tp)
{
	int			n, k, sent, stop = 0;
	char			buf[8192];
 ...
 */
static void
cbheader(const struct hthdr *header, void *arg)
{
 ...
 */
static void
push(struct iobuf *from, struct iobuf *to)
{
	int	n, buflen;
 ...
 */
static int
redirect(const char *prog, int *in, int *out, char **envp)
{
	int	r[2], w[2];
	pid_t	pid;
 ...
 */
static int
spawncgi(struct conn *c, const char *prog)
{
	char	*p, qs[BUFSIZ], gi[BUFSIZ], sn[BUFSIZ], path[BUFSIZ],
		ct[BUFSIZ], rm[BUFSIZ], pl[BUFSIZ], user[BUFSIZ],
		cookie[BUFSIZ], cl[BUFSIZ], *env[20];
	int	ret;
	...
	env[1] = sn;
	(void) snprintf(sn, sizeof(sn), "SCRIPT_NAME=%.*s",
	    c->uri.len, (char *) c->uri.ptr);
	...
	env[3] = qs;
	(void) snprintf(qs, sizeof(qs), "QUERY_STRING=%.*s",
	    c->req.len, (char *) c->req.ptr);

	env[4] = ct;
	(void) snprintf(ct, sizeof(ct), "CONTENT_TYPE=%s", "text/html");

	env[5] = rm;
	(void) snprintf(rm, sizeof(rm), "REQUEST_METHOD=%.*s",
	    c->hti.method.len, (char *) c->hti.method.ptr);
	...
	env[8] = cookie;
	(void) snprintf(cookie, sizeof(cookie), "HTTP_COOKIE=%.*s",
	    c->cookie.len, (char *) c->cookie.ptr);

	env[9] = cl;
	(void) snprintf(cl, sizeof(cl), "CONTENT_LENGTH=%d",
	    vtoint(&c->clength));
	
	env[10] = ct;
	(void) snprintf(ct, sizeof(ct), "CONTENT_TYPE=%.*s",
	    c->ctype.len, (char *) c->ctype.ptr);
...
reply(struct conn *c)
{
	...
	switch (status) {
		case 200:	p = "OK";		break;
		case 302:	p = "Found";		break;
		case 304:	p = "Not Modified";	break;
		case 400:	p = "Bad Request";	break;
		case 401:	p = "Unauthorized";	break;
		case 403:	p = "Forbidden";	break;
		case 404:	p = "Not Found";	break;
		case 408:	p = "Request Timeout";	break;
		case 500:	p = "Internal Error";	break;
		case 501:	p = "Not Implemented";	break;
		case 503:	p = "Service Temporarily Overloaded"; break;
...
static int
decode64(const unsigned char *s, int len, char *to, int tolen)
{
	const unsigned char	*p, *e;
	int			n, d, prev, state;

	for (p = s, e = p + len, n = state = d = prev = 0;
	    n < tolen && p < e; p++, prev = d) {
		if ((d = tab[*p]) == -1)
			continue;
		switch (state) {
		case 0:
 ...
 */
static int
checkauth(struct conn *c, struct vec *uri)
{
	int		authorized = 1;
	const char	*p, *e;
	char		authfile[2048], line[BUFSIZ], auth[BUFSIZ], *pass,
			*cryptpass, *s;
	FILE		*fp;
	...
	if (globhtpass) {
		...
		if (p == NULL)
			p = "/", e = p + 1;
		(void) snprintf(authfile, sizeof(authfile), "%s/%.*s/%s",
		    docroot, e - p, p, htpass);
	}
	...
	if ((fp = fopen(authfile, "r")) != NULL) {
		...
		if (c->auth.len > 6 &&
		    memcmp(c->auth.ptr, "Basic ", 6) == 0 &&
		    decode64((unsigned char *) c->auth.ptr + 6,
		    c->auth.len - 6, auth, sizeof(auth)) > 0 &&
		    (pass = strchr(auth, ':')) != NULL) {
			*pass++ = '\0';
		
			while (fgets(line, sizeof(line), fp) != NULL) {

				/* Trim line */
				for (s = line + strlen(line) - 1; s > line &&
				    isspace(*s); s--)
					*s = '\0';
...
static void
sendheaders(const struct conn *c)
{
	char		headers[2048];
 ...
 */
static int
useindex(char *path, size_t maxpath)
{
	struct stat	st;
...
#define	GETARG(p)							\
	...
	if (dossl) {
		...
		else if (SSL_CTX_use_certificate_file(ctx, certfile,
		    SSL_FILETYPE_PEM) == 0)
