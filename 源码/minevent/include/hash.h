 ...
 * jenkins
 */
static inline uint64_t sw_hash_jenkins(char *key, uint32_t keylen)
{
    uint64_t hashv;
 ...
 */
static inline uint32_t sw_hash_austin(char *key, unsigned int keylen)
{
    unsigned int h, k;
    h = 0 ^ keylen;
 ...
 */
static inline uint64_t sw_hash_php(char *key, uint32_t len)
{
...
uint32_t sw_crc32(char *data, uint32_t size);
